...
@qa_cli.command()
@click.option('--test-path', default='backend/tests/', help='Path to test directory')
@click.option('--auto-fix', is_flag=True, help='Automatically apply fixes')
@click.option('--confidence-threshold', default=85.0, help='Min confidence for auto-fix')
...
def analyze_failures(
    ...
    if not failures:
        click.echo("⚠️  Tests failed but could not parse failures. Run pytest manually for details.")
    ...
    for i, failure in enumerate(failures[:max_fixes], 1):
        click.echo(f"\n{'='*60}")
        click.echo(f"Analyzing failure {i}/{min(len(failures), max_fixes)}: {failure.test_name}")
            ...
            if auto_fix and analysis['confidence'] >= confidence_threshold:
                ...
                if fix_result['applied']:
                    ...
                    try:
                        ...
                        test_file = Path(f"backend/tests/regression/test_{failure.test_name}_regression.py")
