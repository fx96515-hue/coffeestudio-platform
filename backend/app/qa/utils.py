...
def check_coverage() -> dict[str, Any]:
    ...
    return {
        'passed': True,
        'message': 'Coverage check not implemented yet'
    }
...
def check_ruff() -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            'message': 'No issues found' if result.returncode == 0 else f'Issues found: {result.stdout}'
        }
    except Exception as e:
        return {
            'passed': False,
...
def check_mypy() -> dict[str, Any]:
    ...
    try:
        result = subprocess.run(
            ['mypy', '--config-file', 'mypy.ini', 'backend/app'],
            capture_output=True,
            text=True
        )
        ...
        return {
            'passed': result.returncode == 0,
            'message': 'No type errors' if result.returncode == 0 else f'Type errors found: {result.stdout}'
        }
    except Exception as e:
        return {
            'passed': False,
...
def check_bandit() -> dict[str, Any]:
    ...
    try:
        ...
        return {
            'passed': result.returncode == 0,
            'message': 'No security issues' if result.returncode == 0 else f'Security issues found: {result.stdout}'
        }
    except FileNotFoundError:
        return {
            'passed': True,
            'message': 'Bandit not installed (optional)'
        }
    except Exception as e:
        return {
            'passed': False,
...
def check_safety() -> dict[str, Any]:
    ...
    try:
        ...
        return {
            'passed': True,
            'message': 'Safety not installed (optional)'
        }
    except Exception as e:
        return {
            'passed': False,
            'message': f'Error running Safety: {e}'
        }
