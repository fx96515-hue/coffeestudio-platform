...
def upgrade() -> None:
    # Seed freight history data
    base_date = date(2024, 1, 1)
    freight_data = []

    # Generate 80 freight records across different routes and seasons
    routes = [
        ("Callao", "Hamburg"),
        ("Callao", "Rotterdam"),
        ("Santos", "Hamburg"),
    ...
    # Seed coffee price history data
    price_data = []
    ...
    for i in range(150):
        ...
        # Base price varies by quality and processing
        base_price_kg = 5.0
        if grade == "AA":
            base_price_kg = 7.5
        elif grade == "AB":
    ...
    # Seed ML model metadata (placeholder records)
    op.bulk_insert(
        sa.table(
            "ml_models",
