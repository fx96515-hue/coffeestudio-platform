...
def test_update_cooperative(client, auth_headers, db):
    ...
    response = client.patch(f"/cooperatives/{coop.id}", json=update_data, headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
def test_update_nonexistent_cooperative(client, auth_headers):
    ...
    response = client.patch("/cooperatives/99999", json={"name": "New Name"}, headers=auth_headers)
    assert response.status_code == 404
