...
def test_update_source(client, auth_headers, db):
    ...
    source = Source(name="Test Source", url="https://test.com", kind="api", reliability=0.7)
    db.add(source)
    db.commit()
    db.refresh(source)
    
    update_payload = {"reliability": 0.9}
    response = client.patch(f"/sources/{source.id}", json=update_payload, headers=auth_headers)
