...
def test_enrich_entity_not_found(client, auth_headers, db):
    ...
    response = client.post("/enrich/cooperative/99999", json=payload, headers=auth_headers)
...
def test_enrich_cooperative(client, auth_headers, db):
    ...
    with patch("app.services.enrichment.settings") as mock_settings:
        ...
        response = client.post(f"/enrich/cooperative/{coop.id}", json=payload, headers=auth_headers)
...
def test_enrich_unauthorized(client, viewer_auth_headers, db):
    ...
    response = client.post(f"/enrich/cooperative/{coop.id}", json=payload, headers=viewer_auth_headers)
