...
def test_complete_sourcing_workflow(client, auth_headers, db):
    ...
    coop_response = client.post("/cooperatives", json=coop_payload, headers=auth_headers)
    ...
    margin_response = client.post(f"/margins/lots/{lot_id}/runs", json=margin_payload, headers=auth_headers)
...
def test_roaster_sales_workflow(client, auth_headers, db):
    ...
    roaster_response = client.post("/roasters", json=roaster_payload, headers=auth_headers)
    ...
    outreach_response = client.post("/outreach/generate", json=outreach_payload, headers=auth_headers)
...
def test_market_data_workflow(client, auth_headers, db):
    ...
    obs_response = client.post("/market/observations", json=obs_payload, headers=auth_headers)
    ...
    landed_cost_response = client.post("/logistics/landed-cost", json=landed_cost_payload, headers=auth_headers)
...
def test_viewer_access_control_workflow(client, viewer_auth_headers, auth_headers, db):
    ...
    create_response = client.post("/cooperatives", json=create_payload, headers=viewer_auth_headers)
    assert create_response.status_code == 403
    
    # Admin can create
    admin_create_response = client.post("/cooperatives", json=create_payload, headers=auth_headers)
...
def test_duplicate_detection_workflow(client, auth_headers, db):
    ...
    # Suggest duplicates
    dedup_response = client.get("/dedup/suggest?entity_type=cooperative&threshold=80", headers=auth_headers)
