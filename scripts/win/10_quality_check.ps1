...
if (Test-Path (Join-Path $RepoRoot "backend\requirements-dev.txt")) {
  & $Py -m pip install -r (Join-Path $RepoRoot "backend\requirements-dev.txt")
} else {
  & $Py -m pip install ruff mypy pytest pytest-asyncio
}

Write-Host "Ruff (lint)" -ForegroundColor Yellow
& $Py -m ruff check (Join-Path $RepoRoot "backend")

Write-Host "Mypy (typecheck)" -ForegroundColor Yellow
& $Py -m mypy --config-file (Join-Path $RepoRoot "mypy.ini") (Join-Path $RepoRoot "backend\app")
