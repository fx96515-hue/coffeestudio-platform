...
def main() -> int:
    ...
    p.add_argument("--country", type=str, default=None, help="Override country filter (PE|DE)")
    args = p.parse_args()
    ...
    try:
        ...
        if args.both or args.coops:
            out["cooperatives"] = seed_discovery(db, entity_type="cooperative", max_entities=args.max, dry_run=args.dry_run, country_filter=args.country)
        if args.both or args.roasters:
            out["roasters"] = seed_discovery(db, entity_type="roaster", max_entities=args.max, dry_run=args.dry_run, country_filter=args.country)
