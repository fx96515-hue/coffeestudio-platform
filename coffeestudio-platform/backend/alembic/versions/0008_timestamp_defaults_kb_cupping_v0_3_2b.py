...
def upgrade() -> None:
    ...
    op.execute("UPDATE cupping_results SET created_at = now() WHERE created_at IS NULL;")
    op.execute("UPDATE cupping_results SET updated_at = now() WHERE updated_at IS NULL;")
