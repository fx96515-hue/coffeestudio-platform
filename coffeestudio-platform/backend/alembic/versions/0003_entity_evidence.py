...
def upgrade():
    op.create_table(
        ...
        sa.UniqueConstraint("entity_type", "entity_id", "evidence_url", name="uq_entity_evidence"),
    )
    op.create_index("ix_entity_evidence_entity_id", "entity_evidence", ["entity_id"], unique=False)
    op.create_index("ix_entity_evidence_source_id", "entity_evidence", ["source_id"], unique=False)
