...
def _get_or_create_source(db: Session, name: str, url: str | None, kind: str = "api", reliability: float | None = 0.6) -> Source:
        ...
        for e in entities:
            ...
            if k not in deduped:
                ...
                deduped[k]["evidence_urls"] = list(
                    {*(deduped[k].get("evidence_urls") or []), *(e.get("evidence_urls") or [])}
                )

        # Upsert
        now = datetime.utcnow()
        for e in list(deduped.values())[:max_entities]:
            name = e["name"].strip()
            if entity_type == "cooperative":
                ...
                is_new = obj is None
                if is_new:
                    obj = Cooperative(name=name, status="active", next_action="In Recherche")
                    db.add(obj)
                # Fill only if empty (conservative)
                if e.get("region") and not obj.region:
                    obj.region = str(e["region"])[:255]
                if e.get("website") and not obj.website:
                    obj.website = str(e["website"])[:500]
                if e.get("contact_email") and not obj.contact_email:
                    obj.contact_email = str(e["contact_email"])[:320]
                if e.get("notes"):
                    obj.notes = (obj.notes or "").strip()
                    add = str(e["notes"]).strip()
                    if add and add not in (obj.notes or ""):
                        obj.notes = (obj.notes + "\n\n" + add).strip() if obj.notes else add
                obj.meta = obj.meta or {}
                obj.meta.setdefault("discovery", {})
                obj.meta["discovery"].update({"provider": "perplexity", "last_run": now.isoformat()})
                if not dry_run:
                    db.commit()
                    db.refresh(obj)
                ...
                is_new = obj is None
                if is_new:
                    obj = Roaster(name=name, status="active", next_action="In Recherche")
                    db.add(obj)
                if e.get("region") and not obj.city:
                    obj.city = str(e["region"])[:255]
                if e.get("website") and not obj.website:
                    obj.website = str(e["website"])[:500]
                if e.get("contact_email") and not obj.contact_email:
                    obj.contact_email = str(e["contact_email"])[:320]
            ...
            # evidence URLs
            ev_urls = list(dict.fromkeys((e.get("evidence_urls") or [])))
