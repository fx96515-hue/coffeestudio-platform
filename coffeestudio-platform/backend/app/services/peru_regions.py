...
def seed_default_regions(db: Session) -> dict[str, Any]:
    ...
    for r in DEFAULT_REGIONS:
        ...
        if not obj:
            ...
            updated += 1
        for k in ["description_de", "altitude_range", "typical_varieties", "typical_processing", "logistics_notes", "risk_notes"]:
            if r.get(k) and not getattr(obj, k):
                setattr(obj, k, r[k])
    db.commit()
    return {"status": "ok", "created": created, "updated": updated, "total": len(DEFAULT_REGIONS)}
