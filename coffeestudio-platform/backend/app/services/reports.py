...
def _latest_by_key(db: Session, keys: List[str]) -> Dict[str, Optional[MarketObservation]]:
...
def generate_daily_report(db: Session) -> tuple[str, Dict]:
    ...
    coops = (
        ...
        .order_by(Cooperative.total_score.desc().nullslast(), Cooperative.confidence.desc().nullslast(), Cooperative.name.asc())
        .limit(10)
        .all()
    )
    ...
    roasters = (
        ...
        .order_by(Roaster.peru_focus.desc(), Roaster.specialty_focus.desc(), Roaster.name.asc())
        .limit(10)
        .all()
    )
    ...
    md_lines.append(f"- Fracht 40ft (USD): {_fmt_obs(latest.get('FREIGHT:USD_PER_40FT'))}")
    md_lines.append("")

    # Coops
    md_lines.append("## Kooperativen Peru — Top 10")
    if not coops:
        md_lines.append("- (keine Einträge)")
    else:
        for c in coops:
            score = f"{c.total_score:.1f}" if c.total_score is not None else "-"
            conf = f"{c.confidence:.2f}" if c.confidence is not None else "-"
            md_lines.append(f"- **{c.name}** ({c.region or 'Region n/a'}) — Score: {score}, Confidence: {conf}")
            if c.next_action:
                md_lines.append(f"  - Next Action: {c.next_action}")
            if c.requested_data:
                md_lines.append("  - Offene Daten: " + (c.requested_data[:140] + "…" if len(c.requested_data) > 140 else c.requested_data))
    md_lines.append("")

    # Roasters
    md_lines.append("## Röster Deutschland — Auszug")
    if not roasters:
        md_lines.append("- (keine Einträge)")
    else:
        for r in roasters:
    ...
    md_lines.append("- 1) Fehlende Marktdaten ergänzen (FX/C-Preis/Fracht) → verbessert Margenberechnungen")
    md_lines.append("- 2) Top-3 Kooperativen: Kontaktdaten verifizieren + Muster/Lots anfragen")
    md_lines.append("- 3) 3–5 passende Röster identifizieren und Gespräch anbahnen (Peru-/Direct-Trade-Fokus)")

    payload: Dict = {
        "generated_at": now.isoformat(),
        "market": {k: (None if latest[k] is None else {"value": latest[k].value, "observed_at": latest[k].observed_at.isoformat()}) for k in market_keys},
        "top_coops": [
            {"id": c.id, "name": c.name, "region": c.region, "score": c.total_score, "confidence": c.confidence}
            for c in coops
        ],
        "roasters": [
            {"id": r.id, "name": r.name, "city": r.city, "peru_focus": r.peru_focus, "specialty_focus": r.specialty_focus}
            for r in roasters
        ],
    }
