...
def _template(language: Language, *, purpose: Purpose, entity: Any, counterpart: str | None) -> str:
    ...
    if purpose == "sourcing_pitch":
        if language == "de":
            return (
                ...
                "ich baue gerade ein Direct-Trade-Sourcing für Spezialitätenkaffee aus Peru auf. "
                f"Ich bin auf {name} gestoßen{f' ({website})' if website else ''}. "
                "Ich würde gerne kurz verstehen, ob ihr grundsätzlich offen für grüne Rohkaffee-Angebote aus Peru seid "
                "(Microlots/Koop-Lots) und wie euer Prozess für Samples/Preise aussieht.\n\n"
                "Wenn das passt, schicke ich gerne ein kurzes Profil + erste Lot-Optionen (Region/Varietät/Processing) "
        ...
        return (
            ...
            "Estoy construyendo un flujo de abastecimiento direct-trade de café de especialidad desde Perú. "
            f"He encontrado {name}{f' ({website})' if website else ''}. "
            "¿Están abiertos a ofertas de café verde de Perú y cuál es su proceso para muestras y precios?\n\n"
            "Si encaja, puedo enviar un perfil breve y algunas opciones de lotes (región/variedad/proceso) y acordar MOQ/Incoterms.\n\n"
            "Saludos\nCoffeeStudio"
        )
    ...
    if language == "de":
        return (
            f"Hallo {counterpart or 'Team'},\n\n"
            f"ich interessiere mich für eure Kaffees/Projekte in {region or 'Peru'}. "
            "Könntet ihr mir bitte sagen, ob Samples (Rohkaffee) möglich sind und welche Bedingungen gelten "
    ...
    return (
        f"Hola {counterpart or 'equipo'},\n\n"
        "¿Sería posible recibir muestras de café verde y conocer las condiciones "
...
def generate_outreach(
    ...
    *,
    ...
    entity = db.get(Cooperative, entity_id) if entity_type == "cooperative" else db.get(Roaster, entity_id)
    if not entity:
        raise ValueError("entity not found")

    draft = _template(language, purpose=purpose, entity=entity, counterpart=counterpart_name)
    used_llm = False
    ...
    if refine_with_llm and settings.PERPLEXITY_API_KEY:
        ...
        try:
            system = (
                ...
                "Optimiere den folgenden Text für Klarheit, Höflichkeit und Kürze. "
                "Bewahre Fakten, erfinde nichts. Gib NUR den fertigen Text zurück."
            )
    ...
    db.add(EntityEvent(entity_type=entity_type, entity_id=entity_id, event_type="outreach_generated", payload={"language": language, "purpose": purpose, "used_llm": used_llm}))
    db.commit()
