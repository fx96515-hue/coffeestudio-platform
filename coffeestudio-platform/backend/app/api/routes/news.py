...
def list_news(topic: str = "peru coffee", limit: int = 100, days: int = 7, db: Session = Depends(get_db), _=Depends(require_role("admin", "analyst", "viewer"))):
    ...
    return q.order_by(NewsItem.retrieved_at.desc().nullslast()).limit(min(limit, 500)).all()


@router.post("/refresh", response_model=NewsRefreshResponse)
def refresh(topic: str = "peru coffee", country: str = "PE", max_items: int = 25, db: Session = Depends(get_db), _=Depends(require_role("admin", "analyst"))):
