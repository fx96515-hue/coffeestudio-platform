...
def list_coops(db: Session = Depends(get_db), _=Depends(require_role("admin", "analyst", "viewer"))):
    return db.query(Cooperative).order_by(Cooperative.name.asc()).all()


@router.post("/", response_model=CooperativeOut)
def create_coop(payload: CooperativeCreate, db: Session = Depends(get_db), _=Depends(require_role("admin", "analyst"))):
...
def get_coop(coop_id: int, db: Session = Depends(get_db), _=Depends(require_role("admin", "analyst", "viewer"))):
...
def update_coop(coop_id: int, payload: CooperativeUpdate, db: Session = Depends(get_db), _=Depends(require_role("admin", "analyst"))):
    coop = db.query(Cooperative).filter(Cooperative.id == coop_id).first()
    if not coop:
        raise HTTPException(status_code=404, detail="Not found")
...
def delete_coop(coop_id: int, db: Session = Depends(get_db), _=Depends(require_role("admin"))):
    coop = db.query(Cooperative).filter(Cooperative.id == coop_id).first()
    if not coop:
        raise HTTPException(status_code=404, detail="Not found")
    db.delete(coop)
...
def recompute_score(coop_id: int, db: Session = Depends(get_db), _=Depends(require_role("admin", "analyst"))):
    coop = db.query(Cooperative).filter(Cooperative.id == coop_id).first()
    if not coop:
        raise HTTPException(status_code=404, detail="Not found")
