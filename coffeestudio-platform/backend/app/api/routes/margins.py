...
def calc(payload: MarginCalcRequest, _=Depends(require_role("admin", "analyst"))):
    ...
    return MarginCalcResult(computed_at=datetime.now(timezone.utc), inputs=inputs, outputs=outputs)
...
def calc_and_store_for_lot(
    ...
    run = MarginRun(lot_id=lot_id, profile=profile, inputs=inputs, outputs=outputs, computed_at=now)
    db.add(run)
    db.commit()
    db.refresh(run)
    return run
