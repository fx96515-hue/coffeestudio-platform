...
def list_kb(category: str = "logistics", language: str = "de", db: Session = Depends(get_db), _=Depends(require_role("admin", "analyst", "viewer"))):
    q = db.query(KnowledgeDoc).filter(KnowledgeDoc.category == category, KnowledgeDoc.language == language)
