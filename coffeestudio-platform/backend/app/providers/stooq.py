...
def fetch_stooq_last_close(symbol: str, timeout_s: float = 20.0) -> Optional[OhlcQuote]:
    ...
    try:
        r = httpx.get(url, timeout=timeout_s, headers={"User-Agent": "CoffeeStudio/1.0"})
        r.raise_for_status()
        text = r.text
    except Exception:
        return None
    ...
    return OhlcQuote(symbol=symbol, close=close, observed_at=observed_at, source_url=url, raw_text=text)
