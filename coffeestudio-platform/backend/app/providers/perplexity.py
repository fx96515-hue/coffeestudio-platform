...
class PerplexityClient:
    ...
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None, timeout_s: Optional[int] = None):
        ...
        if not self.api_key:
            raise PerplexityError(
                "PERPLEXITY_API_KEY fehlt. Setze ihn in backend/.env oder als Umgebungsvariable."
            )
    ...
    def search(
        ...
        *,
        ...
        if resp.status_code >= 400:
            raise PerplexityError(f"Perplexity /search error {resp.status_code}: {resp.text}")
    ...
    def chat_completions(
        ...
        *,
        ...
        if resp.status_code >= 400:
            raise PerplexityError(f"Perplexity /chat/completions error {resp.status_code}: {resp.text}")
        data = resp.json()
        try:
            return data["choices"][0]["message"]["content"]
        except Exception as e:
            raise PerplexityError(f"Unexpected response shape: {json.dumps(data)[:800]}") from e
...
def safe_json_loads(text: str) -> Any:
    ...
    if "```" in s:
        ...
        if m:
            ...
            s = re.sub(r"```.*?```", lambda mm: mm.group(0).strip("`") , s, flags=re.DOTALL)
            s = s.strip().strip("`")
    ...
    raise ValueError(f"Could not parse JSON from model output (first 300 chars): {s[:300]}")
